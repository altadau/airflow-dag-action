...
with LeucipaDAG(
    ...
    spark_app = LeucipaEmrAddStepsOperator(
        ...
        leucipa_spark_config=LeucipaSparkConfig(
            ...
            leucipa_application="app_pae_asset_hierarchy_export",
            leucipa_config={
                "pg_secret_url_connection": Variable.get(RDS_POSTGRES_PAE_ARG_SECRET_ARN),
                "output_csv_path": f"s3://{Variable.get(TENANT_PAE_ARG_REFINED)}/"
                                   f"assets/csv/hierarchy_data/hierarchy.csv",
                "output_parquet_path": f"s3://{Variable.get(TENANT_PAE_ARG_REFINED)}/"
                                       f"assets/parquet/well_catalog/",
                "output_pump_path": f"s3://{Variable.get(TENANT_PAE_ARG_REFINED)}/"
