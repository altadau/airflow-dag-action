...
with LeucipaDAG(
    ...
    spark_app = LeucipaEmrAddStepsOperator(
        ...
        leucipa_spark_config=LeucipaSparkConfig(
            ...
            leucipa_config={
                "pg_secret_url_connection": Variable.get(RDS_POSTGRES_AGGREGATED_SECRET_ARN),
                "output_csv_path": f"s3://{Variable.get(TENANT_AGGREGATED_REFINED)}/"
                                   f"assets/csv/hierarchy_data/hierarchy.csv",
                "output_parquet_path": f"s3://{Variable.get(TENANT_AGGREGATED_REFINED)}/"
                                       f"assets/parquet/well_catalog/",
                "output_pump_path": f"s3://{Variable.get(TENANT_AGGREGATED_REFINED)}/"
